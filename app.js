...
const roundTitleMap={"CARD DRAW":"札選び","CARD OPEN":"お題との対面","PARENT WORD":"親のひそめごと","WORD OPEN":"言葉のお披露目",DISCUSSION:"宴の推理","WORD SELECT":"推理結果の記帳","SELECTION OPEN":"ひそめごと開帳",RESULT:"宴の顛末",SCORE:"得点の記録"};
...
function historyAnswerMarkup(record){return record.children.map(child=>{const answer=record.answers[String(child.id)]??"";return `<div class="history-answer"><span>${esc(child.name)}</span><span>${esc(answer)}</span><strong class="${answer===record.parentWord?"is-correct":"is-incorrect"}">${answer===record.parentWord?"正解":"不正解"}</strong></div>`;}).join("");}
function historyEntryMarkup(record){const src=cardImagePath(record.image);return `<article class="history-entry"><div class="history-entry-heading"><h3>第${record.round}席</h3><span>親：${esc(record.parentName)}</span></div><div class="history-entry-overview">${src?`<div class="history-card-area"><img class="card-zoom-trigger" src="${esc(src)}" alt="第${record.round}席のお題カード。タップで拡大表示" tabindex="0" role="button"></div>`:""}<div class="history-result"><p class="history-label">親のワード</p><p class="history-parent-word">${esc(record.parentWord)}</p><p class="history-label">4つのワード</p><div class="history-words">${record.words.map(word=>`<span class="word">${esc(word)}</span>`).join("")}</div><p class="history-label">子の回答</p><div class="history-answers">${historyAnswerMarkup(record)}</div><p class="history-summary">${esc(record.summary)}</p></div></div></article>`;}
