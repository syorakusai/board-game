...
document.querySelector("#handoff-redraw-button").onclick=()=>{const p=state.players[state.order[state.parentIndex]];document.querySelector("#round-start-button").disabled=false;document.querySelector("#draw-card").textContent="カードを引く";document.querySelector("#round-card-message").textContent=`${p.name}さん、山札からカードを1枚引いてください。`;show("round");};
