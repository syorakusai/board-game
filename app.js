...
document.querySelector("#handoff-button").onclick=()=>{document.querySelector("#parent-card-area").innerHTML=cardMarkup(state.card);document.querySelector("#official-preview").innerHTML=state.official.map(w=>`<div class="word official">${esc(w.text)}</div>`).join("");document.querySelector("#parent-word").value="";show("parent-input");};
...
document.querySelector("#result-next").addEventListener("click",()=>{if(state.history.some(record=>record.round===state.round))return;const children=state.players.filter((_,i)=>i!==state.order[state.parentIndex]),correct=children.filter(p=>state.answers[p.id]===state.parentCandidateId),summary=correct.length===0?(children.length===1?"子が不正解。得点なし":"全員不正解。親プラス1ポイント"):correct.length===children.length?(children.length===1?"子が正解。子プラス1ポイント":"全員正解。子プラス1ポイント、親マイナス1ポイント"):"一部の子が正解。正解した子に1ポイント";state.history.push({round:state.round,parentName:state.players[state.order[state.parentIndex]].name,image:state.card?.image||"",parentWord:state.parentWord,words:state.words.map(candidate=>candidate.text),answers:{...state.answers},children:children.map(p=>({id:p.id,name:p.name})),summary});});
...
loadCardSetCatalog().catch(()=>{}).finally(()=>show("title"));
