...
document.querySelectorAll("[data-round-title]").forEach((title,index)=>{const row=document.createElement("div"),actions=document.createElement("div"),reactionControl=document.createElement("div"),reactionButton=document.createElement("button"),reactionPalette=document.createElement("div"),historyButton=document.createElement("button"),menu=document.createElement("div"),menuButton=document.createElement("button"),menuPanel=document.createElement("div"),howtoButton=document.createElement("button"),exitButton=document.createElement("button");row.className="screen-title-row";actions.className="screen-title-actions";historyButton.className="history-button";historyButton.type="button";historyButton.textContent="戦績";historyButton.setAttribute("aria-label","戦績を開く");historyButton.onclick=openHistory;menu.className="game-menu";menuButton.className="menu-button";menuButton.type="button";menuButton.textContent="≡";menuButton.setAttribute("aria-label","メニューを開く");menuButton.setAttribute("aria-expanded","false");menuPanel.className="game-menu-panel is-hidden";menuPanel.id=`game-menu-${index}`;menuButton.setAttribute("aria-controls",menuPanel.id);howtoButton.type="button";howtoButton.textContent="遊び方";howtoButton.onclick=openHowto;exitButton.type="button";exitButton.textContent="退出";exitButton.onclick=()=>{if(window.multiplayerPhase1?.requestExit?.())return;returnToTitle();};menuButton.onclick=e=>{e.stopPropagation();document.querySelectorAll(".game-menu").forEach(item=>{if(item!==menu)item.classList.remove("is-open");item.querySelector(".menu-button")?.setAttribute("aria-expanded","false");});const open=menu.classList.toggle("is-open");menuButton.setAttribute("aria-expanded",String(open));};reactionControl.className="reaction-control is-hidden";reactionButton.className="reaction-button";reactionButton.type="button";reactionButton.textContent="💬";reactionButton.setAttribute("aria-label","リアクション");reactionButton.setAttribute("aria-expanded","false");reactionPalette.className="reaction-palette is-hidden";reactionPalette.setAttribute("role","menu");reactionButton.onclick=e=>{e.stopPropagation();window.multiplayerPhase1?.toggleReactionPalette?.(reactionControl);};reactionControl.append(reactionButton,reactionPalette);title.replaceWith(row);row.append(title,actions);menuPanel.append(howtoButton,exitButton);menu.append(menuButton,menuPanel);actions.append(reactionControl,historyButton,menu);});
...
function returnToDrawForRedraw(){const p=state.players[state.order[state.parentIndex]];document.querySelector("#parent-word").value="";document.querySelector("#parent-error").textContent="";state.parentWord="";state.parentCandidateId="";state.words=[];setRoundDrawReady(p);prepareCard();show("round");}
document.querySelector("#handoff-redraw-button").onclick=returnToDrawForRedraw;
document.querySelector("#parent-redraw-button").onclick=returnToDrawForRedraw;
