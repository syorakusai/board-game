...
window.showMultiplayerFinalResults=(players,winners,cards)=>{const screen=document.querySelector('[data-screen="final"]'),recap=document.querySelector("#final-card-recap"),revealDelay=cards.length*300+1280;recap.innerHTML=finalCardRecapMarkup(cards);screen.style.setProperty("--final-reveal-delay",`${revealDelay}ms`);document.querySelector("#final-winners").innerHTML=winners.map(player=>`<p>${esc(player.name)}</p>`).join("");document.querySelector("#final-score-summary").innerHTML=players.map(player=>`<div class="final-score${winners.some(winner=>winner.id===player.id)?" is-winner":""}"><span>${esc(player.name)}</span><strong>${player.score}ポイント</strong></div>`).join("");screen.classList.remove("is-revealing");show("final");void screen.offsetWidth;screen.classList.add("is-revealing");};
