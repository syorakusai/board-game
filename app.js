...
document.querySelector("#round-start-button").onclick=async()=>{document.querySelector("#round-start-button").disabled=true;await chooseCard();document.querySelector("#draw-card").textContent="カードを引きました";document.querySelector("#handoff-card-area").innerHTML=cardMarkup(state.card);handoff(state.players[state.order[state.parentIndex]],"親ワード入力");};
