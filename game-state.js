...
export const state = { playerCount:0, discussionMinutes:2, cardSet:savedCardSet, defaultCardSet:"", wordSet:savedWordSets[savedCardSet]||"", setCatalog:{}, cardSetData:null, wordSets:[], players:[], order:[], parentIndex:0, card:null, official:[], words:[], parentWord:"", answers:{}, answerIndex:0, answerLocked:false, selectedAnswer:"", timer:null, usedCards:new Set(), preparedCard:null, cardPreparation:null, gameSession:0, round:0, handoffNext:"", history:[], currentScreen:"title", howtoReturnScreen:"title" };

export const esc=v=>String(v).replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c]));
export const numberedWordsMarkup=(words,tag="div")=>words.map((word,index)=>`<${tag} class="word${tag==="button"?" word-button":""}"${tag==="button"?` data-word="${esc(word)}"`:""}><span class="word-number" aria-hidden="true">${index+1}</span><span class="word-text">${esc(word)}</span></${tag}>`).join("");
